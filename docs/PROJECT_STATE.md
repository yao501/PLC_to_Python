...
> 最后更新：2026-07-16（**阶段 1 五步扫描编排骨架已通过独立审核**：`WP-20260714-004` 的剩余问题由 `WP-20260714-005` 完整收口，用户已确认两包均为 `CLOSED`；`WP-20260716-006` 已实现确定性单拍编排、稳定运行时导出与 28 项专用测试，当前为 `APPROVED`、待用户确认关闭；OutputPolicy/安全服务/HAL 仍属后续包。AI 协作事件监听已收口两个生产竞态，并新增项目内只读心跳投影，使隔离环境无需访问宿主回环地址即可核验协调器新鲜度；该心跳只证明存活，不授予执行权或旧轮询恢复权。最新 Python 验证记录为协作基础设施 84/84、正式 tests 940/940、0.5 原型 68/68、全仓 1008/1008，**只证明当前 Python 实现与协作基础设施行为，不证明与目标 PLC 语义一致**。）

---

## 1. 项目一句话

把 CODESYS SP16.1 软 PLC 复刻为 Python 原生软 PLC 平台（ST+CFC 双前端 → 语言无关可执行 IR → 扫描引擎），已迁移 14 业务块 + 8 原语作标准库，并建立了正式 L3 IR、静态校验、Store、实例布局、过程映像基础和显式顺序执行器核心（最新全仓测试 1008 项，见 §2），目标是控制+AI 同平台一体化（分进程）。

## 2. 当前位置

- **最新全仓验证**（2026-07-16，项目内协调器心跳投影加入后）：协作基础设施 = **84/84 通过**、正式 tests = **940/940 通过**、0.5 原型 = **68/68 通过**、全仓 = **1008/1008 通过**。
