...
- **下一步（边界分离）**：① 创建并启动**参数装载与启动校验**工作包，只处理静态配置装载、构造参数覆盖和启动期失败关闭，不混入 F2、monitor/watchdog、HAL、真实 I/O 或持久化；② 其后独立推进软件 monitor/周期超时/watchdog 事件源与阶段 1 端到端验收；③ APCM 现场发布前，在隔离的 CODESYS SP16.1 工程中完成导入/编译、整理事件仿真对拍和受控现场趋势确认，现场安全证明保持独立闸门。
