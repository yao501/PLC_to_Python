...
- **下一步（按序）**：按既定路线提出并经用户确认独立 shadow mode 工作包。L2 adapter registry、真实 monitor/HAL 与现场安全证明继续保持后续独立工作包，不混入 shadow mode 范围。
