...
把 CODESYS SP16.1 软 PLC 复刻为 Python 原生软 PLC 平台（ST+CFC 双前端 → 语言无关可执行 IR → 扫描引擎），已迁移 14 业务块 + 8 原语作标准库，并建立了正式 L3 IR、静态校验、Store、实例布局、过程映像基础、显式顺序执行器核心、生产 OutputPolicy、外层故障安全扫描运行器、提交监督器、已审核关闭的 Python shadow mode 核心，以及已完成独立目录验收的 L2 Registry/**22 个 engineering adapter** 纵向链（`WP-20260728-040` 目录验收全绿、待 Codex 审核，本包实跑正式 tests 1383/1383、`prototype_05` 68/68、全仓 1451/1451；历史 1299/1367 等为更早时点快照，原样保留），目标是控制+AI 同平台一体化（分进程）。
...
- **下一步（边界分离）**：① L2 22/22 engineering adapter 目录已由 `WP-20260728-040` 完成独立验收（待 Codex 审核）；下一步是对当前工作区 `WP-026`～`039` 加本包的累积改动做**经用户授权、由 Codex 审核并执行**的 Git/GitHub 收尾（Claude 不做任何 Git 写操作），此后再分别推进 F2、参数装载、软件 monitor/watchdog、真实 HAL 与持久化等独立立项，不混入本目录验收范围；② APCM 现场发布前，在隔离的 CODESYS SP16.1 工程中完成导入/编译、整理事件仿真对拍和受控现场趋势确认；③ 参数装载、软件 monitor/watchdog、真实 HAL 与持久化继续分别立项，现场安全证明保持独立闸门。
