...
> 最后更新：2026-07-16（**阶段 1 五步扫描编排骨架已通过独立审核**：`WP-20260714-004` 的剩余问题由 `WP-20260714-005` 完整收口，用户已确认两包均为 `CLOSED`；`WP-20260716-006` 已实现确定性单拍编排、稳定运行时导出与 28 项专用测试，当前为 `APPROVED`、待用户确认关闭；OutputPolicy/安全服务/HAL 仍属后续包。AI 协作事件监听已收口两个生产竞态：目录/目标文件双 kqueue 监听解决原地写入漏事件，启动就绪握手解决首次写入可早于后台注册的窗口；原地写入+就绪用例连续 10 轮全绿。最新 Python 验证记录为协作基础设施 82/82、正式 tests 938/938、0.5 原型 68/68、全仓 1006/1006，**只证明当前 Python 实现与协作基础设施行为，不证明与目标 PLC 语义一致**。）

---

## 1. 项目一句话

把 CODESYS SP16.1 软 PLC 复刻为 Python 原生软 PLC 平台（ST+CFC 双前端 → 语言无关可执行 IR → 扫描引擎），已迁移 14 业务块 + 8 原语作标准库，并建立了正式 L3 IR、静态校验、Store、实例布局、过程映像基础和显式顺序执行器核心（最新全仓测试 1006 项，见 §2），目标是控制+AI 同平台一体化（分进程）。

## 2. 当前位置

- **最新全仓验证**（2026-07-16，AI 交接监听启动竞态修复后）：协作基础设施 = **82/82 通过**、正式 tests = **938/938 通过**、0.5 原型 = **68/68 通过**、全仓 = **1006/1006 通过**。
- **WP-006 审核证据快照**（2026-07-16，Codex 对 `WP-20260716-006` Round 1 独立复跑）：定向 `tests.test_runtime_engine` = **28/28 通过**、`tests.test_runtime_executor` = **58/58 通过**、`tests.test_runtime_store` = **24/24 通过**、`tests.test_runtime_ir` = **56/56 通过**；当时正式 tests = **937/937 通过**、0.5 原型 = **68/68 通过**、全仓 = **1005/1005 通过**。此后协作基础设施新增 1 项监听器就绪回归测试，因此最新计数比该审核快照各多 1；历史证据保留原始计数，不回写冒充当时结果。以上 Python 测试均不证明与目标 PLC 语义一致。
