...
把 CODESYS SP16.1 软 PLC 复刻为 Python 原生软 PLC 平台（ST+CFC 双前端 → 语言无关可执行 IR → 扫描引擎），已迁移 14 业务块 + 8 原语作标准库，并建立了正式 L3 IR、静态校验、Store、实例布局、过程映像基础、显式顺序执行器核心、生产 OutputPolicy、外层故障安全扫描运行器、提交监督器、已审核关闭的 Python shadow mode 核心，以及已审核收口的 L2 Registry/10 个 engineering adapter 纵向链（最新完整主机快照：正式 tests 1299/1299、`prototype_05` 68/68、全仓 1367/1367），目标是控制+AI 同平台一体化（分进程）。
...
- **下一步（边界分离）**：① 七原语 Git/GitHub 收尾已完成；先为 APCHXHCL/APCSTATISTICS/APCHSFOP/APCHSRATELIM/APCHSACCUM 建立独立 L2 adapter 工作包（10/22→15/22），再为 7 个复杂/组合/授权块建立独立工作包，最终做 22/22 目录验收，不混入 F2、参数装载、HAL 或 monitor；② APCM 现场发布前，在隔离的 CODESYS SP16.1 工程中完成导入/编译、整理事件仿真对拍和受控现场趋势确认；③ 参数装载、软件 monitor/watchdog、真实 HAL 与持久化继续分别立项，现场安全证明保持独立闸门。
