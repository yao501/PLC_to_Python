...
- **下一步（边界分离）**：① APCM 现场发布前，在隔离的 CODESYS SP16.1 工程中完成导入/编译、整理事件仿真对拍和受控现场趋势确认；② 仓库主线按既定路线提出独立 shadow mode 工作包。L2 adapter registry、真实 monitor/HAL 与现场安全证明继续保持后续独立工作包，不混入 shadow mode 首包。
