...
| L5 运行时安全 | 14 | 多数已实现；monitor 为候选 | 多数已合并；monitor 未提交 | 已覆盖/候选 | shadow/OutputPolicy/提交监督已关闭；monitor 候选两项阻塞项已返修、待独立审核 |
| USR 用户入口/后续平台 | 13 | 多数未实现 | 未涉及/试验已合并 | 无/局部 | ST/CFC 前端、导入器、HAL、AI 集成等均未做 |
| ENG 工程支持 | 4 | 已实现（协作机制） | 已合并（主线） | 部分覆盖 | 非软 PLC 产品功能，属协作基建 |

**最新已合并且已关闭的完整主线基线**（唯一权威主机快照）：`WP-20260729-042` 的正式 tests **1441/1441**、`prototype_05` **68/68**、全仓 **1509/1509**（经 [PR #26](https://github.com/yao501/PLC_to_Python/pull/26) 合并，merge `495ebb1…`；PR #27 为行政同步）。monitor 候选环境的 `1548` 计数**不是**已批准主线基线（见 §5）。
...
| L2-04 | L2 组件 | 22/22 engineering adapter 目录 | 8 原语 + 14 业务块完整目录级验收 | [../src/runtime/descriptors/representative.py](../src/runtime/descriptors/representative.py)（`build_default_registry` 注册 22 项）；[../tests/test_runtime_descriptors.py](../tests/test_runtime_descriptors.py)（目录级验收） | 已实现（22/22 目录级验收） | `CLOSED`（WP-20260728-040） | 已合并（主线，[PR #24](https://github.com/yao501/PLC_to_Python/pull/24) merge 8351fdf） | 已覆盖 | 未验证 | 未验证 | `PLATFORM-L2-REGISTRY-1` | L2-01..03 | — |
...
| L2-06 | L2 组件 | F2 缺失变体失败关闭 | `fidelity_f2` 一律 `MissingVariantError` | [../src/runtime/descriptors/registry.py](../src/runtime/descriptors/registry.py)（`MissingVariantError`/`resolve`）；[../src/runtime/loader.py](../src/runtime/loader.py)（装载期消费） | 已实现 | `CLOSED` | 已合并（主线，PR #24） | 已覆盖（全 22 项 fail-closed） | 未验证 | 未验证 | F2 与“块零改动”互斥；是否要 F2 待裁决 | L2-04 | 用户裁决是否上 F2 |
...
| L5-12 | L5 安全 | 软件 monitor（周期/超时/watchdog 事件源） | 可注入整数纳秒时钟、无后台线程的一次性超时事件源 | [../src/runtime/monitor.py](../src/runtime/monitor.py) | **已实现并审核关闭（Python 确定性事件源范围）**：周期观测、阈值锁存、一次性派发、exact-int 边界；公开 `CycleToken` / `WatchdogTimeoutEvent` 字段一律视为不可信，内部以 `_seq` / `_active_start_ns` / `_latched_seq` 保证零观察、状态原子性与防重放 | `CLOSED`（WP-20260729-048 Round 2；WP-043/044/046/047 保留为后续承接前的历史检查点） | **未提交候选**（待本轮 Git/GitHub 收尾） | 已覆盖（monitor 96；正式 tests 1537；全仓 1605；均为当前宿主关闭证据） | 未验证 | 未验证 | `RUNTIME-WATCHDOG`/`RUNTIME-SAFETY-DEFAULT`（deferred/in-progress，不 resolved）；仅软件事件源，不含真实调度/硬件 watchdog | L5-04 | Git/GitHub 收尾；随后阶段 1 真实任务装配另立项 |
...
| ENG-02 | 工程支持 | 工作包状态机 + 协调器 | 事件协调器串行唤醒；WP-048 起新包默认最多 5 轮（历史 3 轮原样保留） | [AI_REVIEW_HANDOFF.md](AI_REVIEW_HANDOFF.md) | 已实现 | 用户协议裁决（2026-07-29） | **协议说明候选未提交**；已合并状态机仍按每包显式 `max_rounds` 通用执行（主线 04e0050） | 部分覆盖（`test_ai_handoff`；状态机无需为 3→5 改码） | 未验证 | 未验证 | 全局执行租约边界；轮次增加不扩大 scope/外部授权；非产品功能 | ENG-01 | 新包显式写 `max_rounds: 5` |
...
6. **当前口径（WP-20260729-048）**：WP-047 Round 3 暴露的 pending 公开事件别名读取已在 WP-048 收口；Claude Round 2 v2 自审/交接合法，Codex Round 2 独立审核 `APPROVED`，用户于 2026-07-29 确认 `CLOSED`。最终宿主证据为 monitor **96/96**、monitor+runner+policy **214/214**、安全运行时组 **240/240**、parameters+executor **192/192**、`test_ai_handoff` **147/147**、正式 tests **1537/1537**、`prototype_05` **68/68**、全仓 **1605/1605**，导入冒烟通过。当前 Git 轴仍是**未提交、未合并**；实际 Git/GitHub 收尾完成前，1605 不得写成主线基线。PLC/CODESYS、真实调度、HAL/物理 I/O、硬件 watchdog 与现场安全仍为未验证。
