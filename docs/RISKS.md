...
| **RUNTIME-SAFETY-DEFAULT** | 扫描异常 / 超时 / 主循环失败时输出落到安全默认值 | blocker | 🟨 in-progress（Python scan-fault/watchdog 安全映像路径已实现；外部失效域未完成） | `OuterScanRunner` 已在提交前扫描异常或显式 watchdog 事件下绕过损坏 request，以两阶段 `SafeImageTicket` 生成/确认全通道安全映像；shadow 下只逻辑采用、不冒充物理提交。真实 watchdog 事件源、进程卡死/OS 崩溃兜底、硬件 watchdog 与现场安全回路仍未完成。 |
