...
1. **BLINK.TIMEHIGH** 端口取 ``500`` ms（= 任务周期 ``cycle_ms``）。源/转换稿
   写 ``T#300MS`` **不是笔误**：``R_TRIG`` 只在任务边界（每 500ms）采样
   ``BLINK.OUT``，亚周期脉宽（300<500）不可分辨，真实 500ms-PLC 中 300 与
   500 等价；本项目 ``BLINK`` 为余数保留实现，取 300 会吞脉冲/抖动，故量化
   到 ``cycle_ms=500`` 才忠实复现（详见 ``APCGCQ-GG4`` / ``APCCD-CD2``）。
   本块以 ``500`` 字面量传入，不暴露为 GCQ 输入。
